...
4. **venue** - Locations for exhibitions and map (Museum, Gallery, Studio, Public Space)
5. **textDocument** - Text content (reviews, statements, catalog essays)
6. **publication** - Books and publications with detailed metadata
7. **timeline** - Timeline entries with media and size options
8. **category** - Hierarchical categorization system
9. **categoryType** - Category type definitions (field-of-art, body-of-work, year, god-mode)
...
### 7. Timeline
```groq
*[_type == "timeline"] | order(date desc) {
  _id,
  title,
  date,
  size,
