...
export default function BrutalistMap({
    ...
    // Live Halle5 coordinates (resolve via Google Places if available)
    const [halle5Coords, setHalle5Coords] = useState<google.maps.LatLngLiteral>(HALLE5_COORDS);
    const halle5PlaceResolvedRef = useRef(false);
    const resolveHalle5Place = useCallback((m?: google.maps.Map) => {
        const mapInst = m || map;
        if (!mapInst || typeof window === 'undefined' || !(window as any).google || halle5PlaceResolvedRef.current) return;
        try {
            const ps = new google.maps.places.PlacesService(mapInst as any);
            ps.findPlaceFromQuery({ query: 'Halle 5 Spinnergasse 1 Dornbirn', fields: ['place_id', 'geometry', 'name', 'formatted_address'] }, (results: any, status: any) => {
                try {
                    if (status === google.maps.places.PlacesServiceStatus.OK && results && results[0] && results[0].geometry && results[0].geometry.location) {
                        ...
                        setHalle5Coords(loc);
                        halle5PlaceResolvedRef.current = true;
                    }
                } catch (e) {
                    // ignore
                }
    ...
    // Keep the Halle5 flag permanently visible as endpoint anchor
    const [showFlag, setShowFlag] = useState(true);

    // Map preloader & staged fly-in sequence
    const [mapPreloading, setMapPreloading] = useState(true);
    const [mapLoadProgress, setMapLoadProgress] = useState(0);
    const hasStartedFlyInRef = useRef(false);
    const tilesLoadedFallbackRef = useRef<number | null>(null);
    ...
    const waitForTilesLoaded = useCallback((m: google.maps.Map, timeout = 1500) => {
        return new Promise<void>((resolve) => {
            ...
            try {
                const listener = m.addListener('tilesloaded', () => {
                    if (!resolved) {
                        resolved = true;
                        clearTimeout(t);
                        // remove the listener
                        (listener as google.maps.MapsEventListener).remove?.();
                        resolve();
                    }
                });
            } catch (e) {
                // if something goes wrong, fallback to timeout
                clearTimeout(t);
                resolve();
            }
        });
    ...
    const onLoad = useCallback(function callback(map: google.maps.Map) {
        ...
        resolveHalle5Place(map);

        // Initial state: Zoomed out, centered on earth
        map.setZoom(3);
    ...
    const onUnmount = useCallback(function callback(map: google.maps.Map) {
        setMap(null);
        hasStartedFlyInRef.current = false;
        if (tilesLoadedFallbackRef.current) { window.clearTimeout(tilesLoadedFallbackRef.current); tilesLoadedFallbackRef.current = null; }
    }, []);

    // Effect to handle route changes from parent
    useEffect(() => {
        if (directionsResponse) {
            ...
            if (map) {
                try {
                    if (directionsResponse.routes && directionsResponse.routes[0] && directionsResponse.routes[0].bounds) {
                        map.fitBounds(directionsResponse.routes[0].bounds, 50);
                    } else if (originCoords) {
                        const bounds = new google.maps.LatLngBounds();
                        bounds.extend(originCoords);
                        bounds.extend(halle5Coords);
            ...
            if (directionsPanelElement) {
                directionsPanelElement.innerHTML = '';
            }
            onRouteStateChange?.(false);
            // Reset view if route cleared
            if (map && !mapPreloading) {
                map.setZoom(17);
                map.panTo(halle5Coords);
            }
        }
    }, [directionsResponse, map, originCoords, directionsPanelElement]);
    ...
    return (
        <div className="relative w-full h-full">
            ...
            {/* Brutalist map preloader */}
            <div className={`absolute top-0 left-0 w-full z-40 pointer-events-none transition-opacity duration-500 ${mapPreloading ? 'opacity-100' : 'opacity-0'}`}>
                <div className="bg-white border-b-4 border-black p-2 flex items-center justify-center">
            ...
            <GoogleMap
                ...
                onLoad={onLoad}
                onUnmount={onUnmount}
                onTilesLoaded={() => startFlyInSequence()}
                options={{
                    styles: MINIMALIST_DARK_STYLE,
                    disableDefaultUI: true, // We'll use our own custom UI
                ...
                <AnimatePresence>
                    {showFlag && !directionsResponse && (
                        ...
                        >
                            ...
                            >
                                {/* High-Contrast Flag Card */}
                                <div className="bg-white border-4 border-black p-4 flex flex-col gap-2 min-w-[220px] shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] relative">
                                    {/* Connection Hook/Stem */}
                                    <div className="absolute bottom-[-16px] left-[-4px] w-8 h-8 bg-white border-l-4 border-b-4 border-black rotate-45 -z-10 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"></div>

                                    <div className="flex items-center gap-3 mb-1 border-b-2 border-black pb-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-auto fill-black" viewBox="0 0 147.4 20.88">
                                            <path d={HALLE5_LOGO_PATH} />
                                        </svg>
                                    </div>
                    ...
                    {originCoords && !directionsResponse && (
                        <OverlayView
                            key={`origin-${originCoords?.lat}-${originCoords?.lng}`}
                        ...
                        >
                            <motion.div
                                ...
                                className="absolute z-50 pointer-events-none origin-bottom-left"
                            >
                                <div className="bg-white border-4 border-black p-3 flex flex-col gap-1 min-w-[200px] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative">
                                    <div className="text-black font-black uppercase text-sm leading-tight">
                                        <p className="text-xs mb-1">Startpunkt</p>
                    ...
                    {/* Permanent small Halle5 marker (native Google Maps Marker anchored to coordinates) */}
                    <Marker
                        position={halle5Coords}
            ...
            {/* Route Input Panel - Top Transitions to Sidebar when active */}
            {/* REMOVED: Route input panel is now handled in the parent component (VisitPage) */}

            {/* Custom Transparent Brutalist Controls */}
            <div className={`absolute bottom-32 right-8 z-20 flex flex-col gap-4 pointer-events-auto items-center transition-opacity duration-500 ${directionsResponse ? 'opacity-30 hover:opacity-100' : 'opacity-100'}`}>
...
.directions-panel .adp-hub-note {
                ...
                /* Map preloader */
                .map-preloader { height: 6px; }
                .map-preloader-bar { height: 6px; width: 0%; }
            `}</style>
        </div>
    );
}