...
export default function ImprintActions() {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      // --- IMPRESSUM ---
      addText(imprintData.impressum.title, 'bold', 16);
      yPos += 5;
      addText(imprintData.impressum.intro, 'normal', 10);
      yPos += 5;

      imprintData.impressum.sections.forEach(section => {
        checkPageBreak(15);
      ...
      yPos += 5;
      imprintData.impressum.copyright.forEach(line => {
        addText(line, 'normal', 8);
      });
      yPos += 10;

      // --- PRIVACY ---
      checkPageBreak(30);
      addText(imprintData.privacy.title, 'bold', 16);
      yPos += 5;
      addText(imprintData.privacy.intro, 'normal', 10);
      yPos += 5;

      imprintData.privacy.sections.forEach(section => {
        checkPageBreak(20);
        addText(section.title, 'bold', 11);
        if (section.content) {
          section.content.forEach(line => {
      ...
      pdf.save('imprint_bildstein_glatz.pdf');
                    ...
                    {imprintData.privacy.sections.map((section, i) => (
                      <div key={i}>
                        ...
                        {section.subsections && section.subsections.map((sub, k) => (
                          <div key={k} className="mt-4 ml-4">
                            <h4 className="font-bold mb-1">{sub.title}</h4>
                            {sub.content.map((line, l) => (
                              <p key={l} className="mb-2 text-sm">{line}</p>
                            ))}
                          </div>
                        ))}
                      </div>
