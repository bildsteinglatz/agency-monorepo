...
const HALLE5_COORDS = { lat: 47.4059666, lng: 9.7444522 };
...
export default function BrutalistMap({ onRouteStateChange }: BrutalistMapProps) {
    ...
    const onLoad = useCallback(function callback(map: google.maps.Map) {
        ...
        // Start Fly-in animation after a short delay
        setTimeout(() => {
            let currentZoom = 2;
            const targetZoom = 17;
            ...
            const zoomInterval = setInterval(() => {
                if (currentZoom < targetZoom) {
                    currentZoom += 0.75; // Faster zoom
                    map.setZoom(Math.floor(currentZoom));
                } else {
                    clearInterval(zoomInterval);

                    // 2. land on location
                    map.panTo(HALLE5_COORDS);

                    // 3. Final cinematic shift: move marker 2/3 to the right
                    // This happens at T=2.9s (approx)
                    setTimeout(() => {
                        map.panBy(-window.innerWidth / 6, 0);

                        // 4. After map shift and info panel fade (at T=5s), show the flag
                        // We wait until the panel is fully in (5s delay + 1.2s duration)
                        setTimeout(() => {
                            setShowFlag(true);
                        }, 3500); // 2.9s + 3.5s = ~6.4s (well after the 5s panel start)
                    }, 800);
                }
            }, 80);

            // Start panning towards center early
            map.panTo(HALLE5_COORDS);

        }, 500); // Shorter start delay
    }, []);
    ...
    const calculateRoute = async () => {
        ...
        try {
            const results = await directionsService.route({
                origin: origin,
                destination: HALLE5_COORDS,
                travelMode: google.maps.TravelMode.DRIVING,
            });
            setDirectionsResponse(results);

            // Notify parent that route is active (to fly out info)
            onRouteStateChange?.(true);

            // Hide the address flag when a route is active to clean up the view
            setShowFlag(false);

            // Center the map but shift for panel
            if (map) {
                map.setZoom(14);
                setTimeout(() => {
    ...
    return (
        <div className="relative w-full h-full">
            ...
            >
                ...
                {directionsResponse && (
                    <DirectionsRenderer
                        ...
                        panel={directionsPanelRef.current || undefined}
                        options={{
                            polylineOptions: {
                    ...
                    {showFlag && !directionsResponse && (
                        ...
                        >
                            <motion.div
                                ...
                                animate={{ scale: 1, opacity: 1, y: -45, x: 25 }}
                                transition={{
                                    type: "spring",
                                    stiffness: 260,
                                    damping: 20,
            ...
            <div className={`absolute z-20 transition-all duration-700 ease-in-out pointer-events-none ${directionsResponse
                    ? 'top-10 left-10 w-[420px]'
                    : 'top-24 left-1/2 -translate-x-1/2 w-[90%] max-w-xl'
                }`}>
                <motion.div
                    initial={{ y: -100, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{
                        delay: directionsResponse ? 0 : 6.5,
                        duration: 1.2,
                        type: "spring",
                        stiffness: 100,
                        damping: 15
                    }}
                    className={`bg-white border-4 border-black p-4 flex flex-col gap-4 shadow-[12px_12px_0px_0px_rgba(255,49,0,1)] pointer-events-auto ${directionsResponse ? 'shadow-[8px_8px_0px_0px_rgba(255,49,0,1)]' : ''
                        }`}
                >
                    <div className="flex flex-col gap-4">
                        <div className="flex-1 relative">
                            ...
                            >
                                <input
                                    ...
                                    className="w-full h-12 bg-black/10 border-2 border-black px-4 font-bold uppercase text-sm focus:bg-white focus:outline-none transition-all placeholder:text-black/50"
                                />
                            </Autocomplete>
                            {origin && (
                                <button
                        ...
                        {!directionsResponse && (
                            <button
                                ...
                                disabled={!origin}
                                className="h-12 bg-[#FF3100] text-white px-8 font-black uppercase hover:bg-black transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                Route
                    ...
                    {/* Step-by-Step Directions Container */}
                    {directionsResponse && (
                        <div className="max-h-[65vh] overflow-y-auto pr-2 custom-scrollbar directions-panel">
                            <div ref={directionsPanelRef} className="directions-content" />
                        </div>
                    )}
                </motion.div>
            </div>
